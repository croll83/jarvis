...
#include "es8311.h"
// driver/gpio.h removed — not needed for ES8311 (I2C-only device)
#include "esp_log.h"
#include "esp_err.h"
#include "esp_check.h"
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_transmit(es->i2c_dev, write_buf, sizeof(write_buf), pdMS_TO_TICKS(1000));
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return i2c_master_transmit_receive(es->i2c_dev, &reg_addr, 1, reg_value, 1, pdMS_TO_TICKS(1000));
}
